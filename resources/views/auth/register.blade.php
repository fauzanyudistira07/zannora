            ...
            <section class="auth-page-shell">
                ...
                <div class="relative z-10 mx-auto flex min-h-screen w-full max-w-[1320px] flex-col px-4 py-5 sm:px-7 lg:px-10">
                    <header class="flex items-center justify-between text-white/95">
                        ...
                        <a href="{{ route('login') }}" class="rounded-xl border border-cyan-100/45 bg-white/10 px-5 py-2 font-semibold text-white shadow-md shadow-cyan-900/20 backdrop-blur-sm transition hover:bg-white/20">Login</a>
                    </header>
                    ...
                    <div class="flex flex-1 items-center justify-center py-6">
                        <div class="auth-card">
                            <h1 class="text-center font-heading text-[2.4rem] font-extrabold tracking-tight text-[#0f3f78] sm:text-[2.7rem]">
                                Create Account
                            </h1>
                            <p class="mt-1 text-center text-xl text-slate-600 sm:mt-2">Join and book your next flight</p>

                            <form method="POST" action="{{ route('register') }}" class="mt-6 space-y-4" x-data="{ showPassword: false, showConfirmPassword: false }">
                                @csrf

                                <label for="name" class="block text-base font-medium text-slate-600">Name</label>
                                ...
                                <label for="phone" class="block text-base font-medium text-slate-600">Phone</label>
                                <div class="auth-field">
                                    <span class="auth-field-icon">
                                    ...
                                    <input id="phone" name="phone" type="text" value="{{ old('phone') }}" autocomplete="tel" placeholder="Phone number (optional)" class="auth-input">
                                    <span class="auth-arrow">&#8250;</span>
                                </div>
                                <x-input-error :messages="$errors->get('phone')" class="text-sm" />
                                ...
                                <label for="password_confirmation" class="block text-base font-medium text-slate-600">Confirm Password</label>
                                <div class="auth-field">
                                    <span class="auth-field-icon">
                                        <svg viewBox="0 0 24 24" class="h-5 w-5 fill-current"><path d="M17 9h-1V7a4 4 0 1 0-8 0v2H7a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-8a2 2 0 0 0-2-2zm-6 7.73V18h2v-1.27a2 2 0 1 0-2 0zM10 9V7a2 2 0 1 1 4 0v2h-4z"/></svg>
                                    </span>
                                    ...
                                    <button type="button" @click="showConfirmPassword = !showConfirmPassword" class="auth-arrow rounded-md px-1.5 transition hover:text-[#123e72]" :aria-label="showConfirmPassword ? 'Hide password confirmation' : 'Show password confirmation'" :title="showConfirmPassword ? 'Hide password confirmation' : 'Show password confirmation'">
                            ...
                            <p class="mt-6 text-center text-[1.2rem] text-slate-600">
                                ...
                                <a href="{{ route('login') }}" class="auth-link-fade font-bold text-[#0f3f78]">Login</a>
                            </p>
