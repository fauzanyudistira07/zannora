    ...
    <section class="space-y-6">
        <form method="GET" class="admin-card space-y-5">
            <div class="admin-section-head">
                <div>
                    ...
                    <p class="admin-section-copy">Filter rentang waktu, airline, rute, dan status agar ringkasan dashboard serta export CSV memakai sumber data yang sama.</p>
                </div>
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
                ...
                <div class="flex flex-wrap items-end justify-between gap-3 border-t border-slate-200/80 pt-4 md:col-span-2 xl:col-span-3 2xl:col-span-6">
                    <p class="text-sm text-slate-500">Export CSV sekarang memuat ringkasan KPI, breakdown status, metode pembayaran, revenue, route populer, dan detail booking.</p>
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <article class="admin-kpi-card">
                ...
                <p class="mt-3 text-3xl font-bold text-slate-800">{{ $summary['total_bookings'] }}</p>
                <p class="mt-2 text-sm text-slate-500">Transaksi booking sesuai filter laporan.</p>
            </article>
            <article class="admin-kpi-card">
                <p class="admin-info-label">Payment Paid</p>
                <p class="mt-3 text-3xl font-bold text-emerald-700">{{ $summary['total_paid_payments'] }}</p>
                <p class="mt-2 text-sm text-slate-500">Pembayaran berhasil tervalidasi.</p>
            </article>
            <article class="admin-kpi-card">
                <p class="admin-info-label">Cancelled Booking</p>
                <p class="mt-3 text-3xl font-bold text-red-600">{{ $summary['total_cancelled_bookings'] }}</p>
                <p class="mt-2 text-sm text-slate-500">Booking yang dibatalkan di periode ini.</p>
            </article>
            <article class="admin-kpi-card">
                <p class="admin-info-label">Flight Aktif</p>
                <p class="mt-3 text-3xl font-bold text-slate-800">{{ $summary['total_active_flights'] }}</p>
                <p class="mt-2 text-sm text-slate-500">Status flight scheduled atau delayed.</p>
            </article>
            <article class="admin-kpi-card">
                <p class="admin-info-label">Pendapatan</p>
                <p class="mt-3 text-3xl font-bold text-emerald-700">Rp{{ number_format((float) $summary['revenue_total'], 0, ',', '.') }}</p>
