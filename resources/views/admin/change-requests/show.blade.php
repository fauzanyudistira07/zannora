...
@section('title', 'Change Request Detail | Zannora')
        ...
        <article class="admin-card space-y-5">
            <div class="admin-section-head">
                <div>
                    ...
                    <p class="admin-section-copy">Dibuat {{ $changeRequest->created_at?->format('d M Y H:i') }} oleh {{ $changeRequest->user?->name }}</p>
                </div>
            ...
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                ...
                <article class="admin-info-card">
                    ...
                    <p class="admin-info-value">
                        {{ $changeRequest->booking?->flight?->departureAirport?->code ?: '-' }}
                        ->
                        {{ $changeRequest->booking?->flight?->arrivalAirport?->code ?: '-' }}
                    </p>
                ...
                <article class="admin-info-card">
                    ...
                    @if ($changeRequest->preferredFlight)
                        ...
                        <p class="mt-2 text-sm text-slate-500">
                            {{ $changeRequest->preferredFlight->departureAirport?->code }} -> {{ $changeRequest->preferredFlight->arrivalAirport?->code }}
                        </p>
                    @else
            ...
            <article class="admin-card-soft">
                ...
                <p class="mt-2 text-slate-700">{{ $changeRequest->reason }}</p>
            </article>
        </article>

        <article class="admin-card">
            <h2 class="font-heading text-xl font-bold text-[#0f3f78]">Process Request</h2>
            ...
            @if ($changeRequest->processedByUser)
                <div class="mt-4 rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
                    Last processed by <strong>{{ $changeRequest->processedByUser->name }}</strong>
                    at {{ $changeRequest->processed_at?->format('d M Y H:i') ?: '-' }}.
                </div>
            @endif
        </article>
    </section>
