...
<article class="portal-card min-w-0 overflow-hidden print:break-inside-avoid">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0">
            <p class="text-sm uppercase tracking-[0.25em] text-[#315b8c]">E-Ticket</p>
            <h2 class="ticket-number mt-2 max-w-full break-all font-heading font-bold text-slate-800">{{ $detail->ticket_number ?: 'Ticket' }}</h2>
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-[1fr_280px]">
        <div class="portal-card-soft">
            ...
            <div class="mt-4 flex items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white/80 p-6">
        ...
        <div class="flex flex-col gap-3">
            <a href="{{ route('tickets.pdf', $ticket) }}" class="portal-btn-gold portal-print-hide w-full">Download PDF</a>
            @if (in_array($detail->boarding_status, ['checked_in', 'boarded'], true))
                <a href="{{ route('my-bookings.checkin.pdf', [$booking, $detail]) }}" class="portal-btn-blue portal-print-hide w-full">Boarding Pass PDF</a>
            @elseif (in_array($booking->status, ['confirmed', 'completed'], true))
                <a href="{{ route('my-bookings.checkin.index', $booking) }}" class="portal-btn-blue portal-print-hide w-full">Open Check-In</a>
            @endif
            @if ($showPrintButton)
                <button type="button" onclick="window.print()" class="portal-btn-blue portal-print-hide w-full">Print</button>
            @endif
            @if (($showSingleLink ?? false) === true)
                <a href="{{ route('tickets.show', $ticket) }}" class="portal-btn-blue portal-print-hide w-full">Open Single Ticket</a>
            @endif
        </div>
    </div>
</article>
