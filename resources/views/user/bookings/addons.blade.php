...
@section('title', 'Zannora | Add-Ons')
@section('active', 'bookings')

@section('content')
    <section class="space-y-6" x-data="addonForm({{ Js::from($catalog) }})">
        <article class="portal-card">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="min-w-0">
                    <p class="text-sm uppercase tracking-[0.25em] text-[#315b8c]">Booking Add-Ons</p>
        ...
        <div class="grid gap-6 xl:grid-cols-[1fr_400px]">
            <article class="portal-card">
                <h2 class="font-heading text-2xl font-bold text-[#0f3f78]">Tambah Add-On</h2>
                <p class="mt-1 text-sm text-slate-600">Pilih add-on per passenger atau per booking. Harga otomatis masuk ke tagihan pending.</p>

                <form method="POST" action="{{ route('my-bookings.addons.store', $booking) }}" class="mt-5 space-y-4">
                    @csrf
                    <div>
                    ...
                    <div>
                        <label class="portal-label" for="booking_detail_id">Passenger (Opsional untuk booking-level add-on)</label>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            <label class="portal-label" for="notes">Notes (Opsional)</label>
            ...
            <aside class="portal-card h-fit">
                <h2 class="font-heading text-2xl font-bold text-[#0f3f78]">Ringkasan</h2>
        ...
        <article class="portal-card">
            <h2 class="font-heading text-2xl font-bold text-[#0f3f78]">Daftar Add-On</h2>
            <div class="mt-4 space-y-3">
                    ...
                    <div class="portal-card-soft">
                        <div class="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <div class="text-right">
                                ...
                                <div class="mt-1">@include('admin.partials.status-badge', ['status' => $addon->status])</div>
                                @if ($addon->status === 'selected')
                                    <form method="POST" action="{{ route('my-bookings.addons.destroy', [$booking, $addon]) }}" class="mt-2">
                                        @csrf
                                        @method('DELETE')
                ...
                @empty
                    <p class="portal-card-soft text-slate-500">Belum ada add-on pada booking ini.</p>
                @endforelse
            </div>
        </article>
    </section>
