...
@section('title', 'Zannora | QRIS Completed')
@section('active', 'bookings')

@section('content')
    <section class="mx-auto max-w-3xl space-y-6">
        <article class="portal-card text-center">
            ...
            <p class="mt-5 text-sm uppercase tracking-[0.25em] text-[#315b8c]">Payment Completed</p>
            <h1 class="mt-2 font-heading text-3xl font-bold text-[#0f3f78] sm:text-4xl">{{ $paymentLabel }}</h1>
            <p class="mt-3 text-lg text-slate-600">Pembayaran demo berhasil diproses otomatis.</p>
            <p class="mt-2 text-sm text-slate-500">Nilai yang diterima QRIS: Rp{{ number_format((float) $payment->amount, 0, ',', '.') }}</p>
            <p class="mt-6 text-3xl font-bold text-slate-800">Rp{{ number_format((float) $payment->amount, 0, ',', '.') }}</p>
        </article>
