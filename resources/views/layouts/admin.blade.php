...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="portal-shell font-sans antialiased text-slate-700">
        ...
        <div class="portal-container relative z-10 py-6 lg:py-8" x-data="{ sidebarOpen: false }">
            <div
            ...
            <div class="admin-shell">
                <aside class="admin-sidebar" :class="{ 'admin-sidebar-open': sidebarOpen }">
                    <a href="{{ route('admin.dashboard') }}" class="portal-brand">
                        ...
                        <span>
                            ...
                            <small class="portal-brand-subtitle">{{ $backofficeUser->roleLabel() }} Workspace</small>
                        </span>
                    </a>
                ...
                <div class="admin-content">
                    <header class="admin-topbar">
                        <div>
                            <p class="text-xs uppercase tracking-[0.2em] text-[#315b8c]">{{ $backofficeUser->roleLabel() }} Control</p>
                            <h1 class="font-heading text-2xl font-bold text-slate-800">@yield('page-title', 'Dashboard')</h1>
                        </div>
                        <div class="flex min-w-0 items-center gap-2">
                            <span class="max-w-[230px] truncate rounded-full border border-blue-200 bg-white px-3 py-1 text-sm text-slate-600 sm:max-w-none">
                                {{ $backofficeUser->name }} Â· {{ $backofficeUser->roleLabel() }}
                            </span>
