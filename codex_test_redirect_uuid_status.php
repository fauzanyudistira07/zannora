...
$path = parse_url((string)$p->midtrans_redirect_url, PHP_URL_PATH) ?: '';
$uuid = basename((string)$path);
...
$res = Illuminate\Support\Facades\Http::acceptJson()->withBasicAuth($key, '')->withOptions(['verify'=>(bool)config('services.midtrans.verify_ssl',true)])->get($base.'/v2/'.$uuid.'/status');
$j=(array)$res->json();
